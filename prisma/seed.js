...
async function seedAdditionalServices() {
  ...
  console.log(`✅ Additional services upserted: ${additionalServicesData.length}`)
}

async function seedCaseStudyAndContent() {
  console.log('📝 Seeding sample case study (idempotent)...')
  const title =
    "From Idea to Income: Margarita's Podcast Success"
...
async function seedServiceTypesAndSamples() {
  ...
  const serviceTypes = [
    { name: 'Podcast Production', description: 'Professional recording services' },
    { name: 'Reels Production', description: 'Post-production editing services' },
    { name: 'Media post-production', description: 'Live streaming and broadcast services' },
    { name: 'Social media management', description: 'Social media strategy and content creation' },
    { name: 'Beneficial packages', description: 'Video production for marketing and branding' },
  ]
