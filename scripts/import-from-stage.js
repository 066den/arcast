...
async function importAdditionalServices() {
  ...
  for (const a of list || []) {
    ...
    if (existing) {
      await prisma.additionalService.update({ where: { id: existing.id }, data })
    } else {
...
async function importPackages() {
  ...
  for (const p of list) {
    const name = p.name
    const existing = await prisma.package.findFirst({ where: { name }, select: { id: true } })
    const data = {
      name,
...
async function importServiceTypesFromSamples(samples) {
  for (const s of samples) {
    if (s.serviceType?.name) {
      await upsertServiceTypeByName(s.serviceType.name, s.serviceType.description || null)
    }
  }
}
...
async function importSamples() {
  ...
  for (const s of list || []) {
    const name = s.name
    const existing = await prisma.sample.findFirst({ where: { name }, select: { id: true } })
    let serviceTypeId = null
    if (s.serviceType?.name) {
      serviceTypeId = await upsertServiceTypeByName(s.serviceType.name, s.serviceType.description || null)
    }
    const data = {
      name,
...
async function importCaseStudies() {
  ...
  for (const c of list || []) {
    const title = c.title
    const existing = await prisma.caseStudy.findFirst({ where: { title }, select: { id: true } })
    let clientId = null
    if (c.client?.name) {
    ...
    for (const st of c.staff || []) {
      const id = await upsertStaffByName(st.name, { role: st.role || null, imageUrl: st.imageUrl || null })
      if (id) connectStaff.push({ id })
    }
    const connectEquipment = []
    for (const eq of c.equipment || []) {
      const id = await upsertEquipmentByName(eq.name, { description: eq.description || null, imageUrl: eq.imageUrl || null })
    ...
    let caseId
    if (existing) {
      caseId = (await prisma.caseStudy.update({ where: { id: existing.id }, data, select: { id: true } })).id
    } else {
      caseId = (await prisma.caseStudy.create({ data, select: { id: true } })).id
    }
    // sync caseContent by unique (title, order)
    for (const content of c.caseContent || []) {
      const where = { caseStudyId: caseId, title: content.title || '', order: content.order ?? 0 }
      const has = await prisma.caseStudyContent.findFirst({ where, select: { id: true } })
      const cd = {
        caseStudyId: caseId,
        title: content.title || '',
      ...
      if (has) {
        await prisma.caseStudyContent.update({ where: { id: has.id }, data: cd })
      } else {
...
async function importBlog() {
  ...
  for (const b of list || []) {
    const existing = await prisma.blogRecord.findFirst({ where: { title: b.title }, select: { id: true } })
    const data = { title: b.title, tagline: b.tagline || '', mainText: b.mainText || '' }
    if (existing) {
...
async function importDiscountCodes() {
  ...
  for (const d of list || []) {
    ...
    const data = {
      ...
      endDate: d.endDate ? new Date(d.endDate) : new Date(Date.now() + 86400000),
      usageLimit: d.usageLimit ?? null,
      usedCount: d.usedCount ?? 0,
      firstTimeOnly: !!d.firstTimeOnly,
      minOrderAmount: d.minOrderAmount ? Number(d.minOrderAmount) : null,
      applicableContentTypes: Array.isArray(d.applicableContentTypes) ? d.applicableContentTypes : [],
    }
    if (existing) {
